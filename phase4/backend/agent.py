...
# ============================================================================
# Raw Implementation Functions (callable directly for fallback)
# ============================================================================

def _add_task_impl(title: str, description: str = "") -> dict:
    """Raw implementation of add_task."""
...
def _list_tasks_impl(status: str = "all") -> dict:
    """Raw implementation of list_tasks."""
    from models import Task
...
def _complete_task_impl(task_id: int) -> dict:
    """Raw implementation of complete_task."""
    from models import Task
...
def _delete_task_impl(task_id: int) -> dict:
    """Raw implementation of delete_task."""
    from models import Task
...
def _update_task_impl(task_id: int, title: str = None, description: str = None) -> dict:
    """Raw implementation of update_task."""
    from models import Task
    from datetime import datetime
...
# ============================================================================
# Function Tools for Agents SDK (wrap implementations)
# ============================================================================
...
def add_task(title: str, description: str = "") -> dict:
    ...
    Args:
        ...
        description: Optional description for the task (default: empty string)

    Returns:
        dict with status, data (task_id, title), and message
    """
    return _add_task_impl(title, description)
...
def list_tasks(status: str = "all") -> dict:
    ...
    """
    return _list_tasks_impl(status)
...
def update_task(task_id: int, title: str = "", description: str = "") -> dict:
    ...
    Args:
        ...
        title: New title for the task (default: empty string, meaning no change)
        description: New description for the task (default: empty string, meaning no change)

    Returns:
        dict with status, data, and message
    """
    # Convert empty strings to None for the implementation
    title_val = title if title else None
    desc_val = description if description else None
    return _update_task_impl(task_id, title_val, desc_val)
...
async def fallback_process(
    ...
    def get_task_by_position(position: str) -> Optional[int]:
        result = _list_tasks_impl(status="all")
    ...
    for cmd in commands:
        ...
        for pattern in add_patterns:
            ...
            if match:
                ...
                if title:
                    result = _add_task_impl(title=title)
                    tool_calls_made.append(ToolCallResult(
                        tool="add_task",
            ...
            if any(word in cmd_lower for word in ["show", "list", "what", "see", "my task", "tasks", "todo"]):
                ...
                result = _list_tasks_impl(status=status)
                tool_calls_made.append(ToolCallResult(
                    tool="list_tasks",
            ...
            # COMPLETE TASK intent
                ...
                if task_id:
                    result = _complete_task_impl(task_id=task_id)
                    tool_calls_made.append(ToolCallResult(
                        tool="complete_task",
            ...
            # DELETE TASK intent
                ...
                if task_id:
                    result = _delete_task_impl(task_id=task_id)
                    tool_calls_made.append(ToolCallResult(
                        tool="delete_task",
                        parameters={"task_id": task_id},
                        result=result
            ...
            # UPDATE TASK intent
                ...
                if numbers:
                    ...
                    if match:
                        ...
                        result = _update_task_impl(task_id=task_id, title=new_title)
                        tool_calls_made.append(ToolCallResult(
                            tool="update_task",
